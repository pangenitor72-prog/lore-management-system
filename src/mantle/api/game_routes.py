...
from src.mantle.api.helpers.prompt_builders import (
    get_story_scope_guidance as _get_story_scope_guidance,
    get_genre_guidance as _get_genre_guidance,
    get_style_instructions as _get_style_instructions,
    get_protagonist_arc_guidance as _get_protagonist_arc_guidance,
    get_lethality_guidance as _get_lethality_guidance,
    get_moral_complexity_guidance as _get_moral_complexity_guidance,
    build_storytelling_preferences_context,
    get_guidance_instruction as _get_guidance_instruction,
)
from src.mantle.api.helpers.narrative_extraction import (
    extract_entity_names_from_text as _extract_entity_names_from_text,
...
# Wrapper for storytelling preferences that passes module-level LORE_BASES
def _build_storytelling_preferences_context(session: Dict[str, Any]) -> str:
    """Build storytelling preferences context, passing LORE_BASES from this module."""
    return build_storytelling_preferences_context(session, LORE_BASES)
...
# This allows other route modules (save_routes, etc.) to share session data.
...
# NOTE: The following functions were extracted to helper modules:
# - _get_story_scope_guidance -> src/mantle/api/helpers/prompt_builders.py
# - _extract_entity_names_from_text -> src/mantle/api/helpers/narrative_extraction.py
# - _extract_scene_npcs_from_narrative -> src/mantle/api/helpers/narrative_extraction.py
# - _get_npc_ids_for_memory_context -> src/mantle/api/helpers/narrative_extraction.py
# - _apply_legend_reputation_to_new_npcs -> src/mantle/api/helpers/narrative_extraction.py
# - _get_genre_guidance -> src/mantle/api/helpers/prompt_builders.py
# - _get_style_instructions -> src/mantle/api/helpers/prompt_builders.py
# - _get_protagonist_arc_guidance -> src/mantle/api/helpers/prompt_builders.py
# - _get_lethality_guidance -> src/mantle/api/helpers/prompt_builders.py
# - _get_moral_complexity_guidance -> src/mantle/api/helpers/prompt_builders.py
# - _build_storytelling_preferences_context -> src/mantle/api/helpers/prompt_builders.py
# - _get_guidance_instruction -> src/mantle/api/helpers/prompt_builders.py
# These are imported at the top of this file with their original names.
...
# NOTE: _get_guidance_instruction moved to helpers/prompt_builders.py
...
- Skills: {', '.join(dnd_char.skill_proficiencies)}{powers_block}{features_block}{equip_block}{identity_block}{personality_block}"""
    ...
    # Extract NPCs from narrative and update scene_npcs for memory context in next turn
    # This enables proper relationship tracking - DM will know about NPC relationships
    if narrative and game_state and db:
        try:
            world_id = session.get("session_world_id") or session.get("world_id")
            if world_id:
                scene_npc_ids = await _extract_scene_npcs_from_narrative(
                    narrative=narrative,
                    world_id=world_id,
                    db=db,
                )
                if scene_npc_ids:
                    # Track previous scene NPCs to identify new encounters
                    previous_npcs = set(game_state.scene_npcs) if game_state.scene_npcs else set()
                    new_npcs = [npc_id for npc_id in scene_npc_ids if npc_id not in previous_npcs]

                    game_state.set_scene_npcs(scene_npc_ids)
                    logger.debug(f"[MEMORY] Updated scene NPCs for next turn: {scene_npc_ids}")

                    # Apply legend reputation to NPCs meeting the player for the first time
                    memory_manager = session.get("memory_manager")
                    if new_npcs and memory_manager:
                        _apply_legend_reputation_to_new_npcs(new_npcs, memory_manager)
        except Exception as e:
...
# GRAPH VISUALIZATION
# ============================================================
# NOTE: Graph routes have been moved to graph_routes.py
# They are now accessible at /api/graph/* (new) and /api/game/graph/* (legacy)
